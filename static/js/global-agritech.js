if (window.innerWidth <= 768) {
    ...
    const sellerDiv = document.querySelectorAll('.seller-name');
    const overlayDiv = document.querySelectorAll('.overlay-box');
    sellerDiv.forEach(seller => {
      seller.addEventListener('click', function(event){
        overlayDiv.forEach(div =>{
          if (div.id === event.target.id){
            div.style.display='block';
            div.children[0].style.display='block';
            const removeI = document.getElementById(`${event.target.id}-i`);
            removeI.addEventListener('click', function(event){
              console.log(div.id, event.target.id)
              if (div.id === event.target.id.split('-')[0]){
    ...
    const countriesDiv = document.querySelectorAll('.category');
    const categoryDiv = document.querySelector('#tabs');
    const selectedCountryDiv =document.querySelector('.selected-category');
    for(let i=0; i<categoryDiv.children.length; i++){
      if(categoryDiv && categoryDiv.children[i] && selectedCountryDiv.id.split('_')[0] === categoryDiv.children[i].id){
        categoryDiv.children[i].style.backgroundImage='linear-gradient(180deg, darkblue, lightblue)';
      }
    }
    countriesDiv.forEach(c=>{
      c.style.display='none';
    });
    const selectedCat = document.querySelector('.selected-product-category');
    selectedCat.style.background='linear-gradient(to left,rgb(1, 46, 1), green, rgb(1, 46, 1))';
    selectedCat.style.color='white';
    selectedCat.style.borderRadius='10px 0';
    selectedCat.style.borderBottom='2px solid goldenrod'
    function clikToOpen(event){
      const selectedDiv = document.querySelector(`#${event.target.id}_div`)
      for(let i=0; i<countriesDiv.length; i++){
        if(countriesDiv[i] === selectedDiv){
          countriesDiv[i].style.display='block';
          selectedCountryDiv.style.display='none';
          categoryDiv.children[i].style.backgroundImage='linear-gradient(180deg, darkblue, lightblue)';
        }else{
          if(selectedDiv.id === selectedCountryDiv.id){
            countriesDiv[i].style.display='none';
            selectedCountryDiv.style.display='block';
            if(selectedDiv.id.split('_')[0]===categoryDiv.children[i].id){
              categoryDiv.children[i].style.backgroundImage='linear-gradient(180deg, darkblue, lightblue)';
            }else{
